...
export default function CreatorRequestScreen({ navigation }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const unsubscribe = listenMyCreatorRequest(user.uid, setRequest);
    return unsubscribe;
  }, [user.uid]);
  ...
  const handleSubmit = async () => {
    if (!reason.trim()) {
      Alert.alert("Begründung fehlt", "Bitte kurz erklären, warum du Creator werden möchtest.");
      return;
    }
    setSubmitting(true);
    try {
      await submitCreatorRequest(user.uid, {
  ...
  if (user?.isCreator) {
    return (
      <View style={styles.container}>
        <ScreenHeader onBack={() => navigation.goBack()} title="Creator werden" />
        <View style={styles.centerState}>
          <CreatorBadge size={40} />
          <Text style={styles.centerTitle}>Du bist bereits Creator</Text>
          <TouchableOpacity onPress={() => navigation.replace("CreatorStudio")}>
            <Text style={styles.studioLink}>Zum Creator-Studio</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.introRow}>
          <Text style={styles.intro}>
            Der Creator-Modus schaltet ein eigenes Creator-Studio mit Inhalts-Statistiken und ein
            goldenes Abzeichen auf deinem Profil frei. Erzähl kurz, warum du Creator werden
            möchtest, und hinterlege optional Links (z. B. andere Profile, Portfolio). Der Gründer
            sieht sich jede Anfrage persönlich an.
          </Text>
          <BetaBadge />
        </View>
        ...
        <TextInput
          ...
          placeholder="Warum solltest du Creator werden?"
