...
import { listenPendingVerificationRequests, reviewVerificationRequest } from "../services/verificationService";
...
export default function FounderVerificationRequestsScreen({ navigation }) {
  const [requests, setRequests] = useState([]);
  const [busyUid, setBusyUid] = useState(null);

  React.useEffect(() => {
    const unsubscribe = listenPendingVerificationRequests(setRequests);
    return unsubscribe;
  }, []);

  const handleReview = (uid, approve) => {
    Alert.alert(
      approve ? "Verifizierung annehmen?" : "Verifizierung ablehnen?",
      approve
        ? "Diese Person erhält sofort den blauen Haken."
      ...
      [
        { text: "Abbrechen", style: "cancel" },
        {
          ...
          onPress: async () => {
            setBusyUid(uid);
            try {
              await reviewVerificationRequest(uid, { approve });
            } catch (e) {
  ...
  return (
    <View style={styles.container}>
      <ScreenHeader onBack={() => navigation.goBack()} title="Verifizierungs-Anfragen" />
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.intro}>
          Offene Anfragen auf den blauen Haken. Annehmen setzt sofort den verified-Status.
        </Text>
